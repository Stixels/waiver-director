...
function buildPrompt(args: EmailAIRequest & { sanitizedBody: string; workspaceName: string }) {
	...
	return [
		...
		'Keep rationale to one short sentence, at most 25 words, explaining the main improvement. Do not recap the whole review or describe your process.',
		'Include at most 2 issues and 2 suggestions, each at most 12 words. Mention only useful, specific changes; empty lists are fine when the draft already works.',
		...
		'Keep the proposed email brief and natural. Preserve what already works; do not add extra paragraphs, repeated links, or sign-offs unless needed for the goal. Do not invent claims about speed, ease, check-in, or how often a waiver must be signed. Use only facts provided in the draft or goal.',
