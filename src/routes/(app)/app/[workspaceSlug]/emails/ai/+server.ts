...
function buildPrompt(args: EmailAIRequest & { sanitizedBody: string; workspaceName: string }) {
	...
	return [
		...
		'Write review feedback for a busy business owner, not an email marketer. Use everyday words and a warm, direct tone.',
		'Write rationale as a helpful short paragraph of 2–3 sentences: explain what already works, the main opportunity to improve, and why the proposed changes help. Aim for about 40–70 words when there is enough to discuss; do not pad a simple review.',
		'Include a few specific issues and suggestions when useful, usually 1–3 of each. Use complete, natural sentences that connect a concrete observation or change to its benefit. Give enough context to act on the advice, rather than terse labels. Empty lists are fine when there is nothing useful to add.',
		'Avoid jargon such as CTA, merge fields, benefit-led framing, deliverability, and engagement patterns. Say things like "Make the next step clearer" or "Use a friendlier greeting" instead.',
		'Do not repeat the same feedback across rationale, issues, and suggestions. Do not speculate about spam filters or claim results you cannot verify.',
		'Keep the proposed email natural and appropriately detailed for the goal. Preserve what already works and avoid filler or repeated links. Do not invent claims about speed, ease, check-in, or how often a waiver must be signed. For example, never add "takes a minute", "quick to do", or "only needs doing once" unless the draft or goal explicitly states that fact.',
