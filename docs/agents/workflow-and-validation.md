...
- Branch from the latest `origin/main` and include the Linear identifier in the branch name. Prefer the issue's `gitBranchName` when Linear provides one; otherwise Codex branches use `codex/<issue-id>-<short-slug>`.
